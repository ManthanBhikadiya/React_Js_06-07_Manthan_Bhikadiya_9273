...
const ProductCard = ({ data, mode = "product" }) => {
  ...
  const handleWishlistToggle = () => {
    if (isInWishlist) {
      disptch(removeFromWishlist(data.id));
    } else {
      disptch(addToWishlist(data));
    }
  };
  ...
  return (
    <div className="pt-10">
      <div className="w-[300px] max-w-sm bg-neutral-primary-soft p-6 border border-default rounded-base shadow-xs">
        <div className="cursor-pointer" onClick={handleWishlistToggle}>{isInWishlist ? (
          <FaHeart className="text-red-500" />
        ) : (
          <CiHeart className="text-gray-400 hover:text-red-400" />
        )}</div>
        <a href="#">
          <img
